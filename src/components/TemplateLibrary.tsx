...
import { Search, Plus, Star, Download, Upload, Eye, Copy, Filter, Sparkles, TrendingUp, Users, Zap } from 'lucide-react';
...
const CATEGORIES = ['Lead Generation', 'Product Sales', 'SaaS Signup', 'E-commerce', 'Event Registration', 'Newsletter', 'App Download', 'Service Booking'];
const INDUSTRIES = ['Technology', 'Healthcare', 'Finance', 'Education', 'E-commerce', 'Real Estate', 'Marketing', 'Manufacturing', 'Professional Services'];
export const TemplateLibrary: React.FC<TemplateLibraryProps> = ({
    ...
    try {
      ...
      switch (sortBy) {
        case 'usage':
          ...
          break;
        case 'conversion':
          query = query.order('conversion_rate', {
            ascending: false
          });
          break;
        case 'recent':
  ...
  const createTemplate = async (fromPageId?: string) => {
    try {
      ...
      if (fromPageId) {
        ...
        const {
          data: page
        } = await supabase.from('generated_pages').select('content, sections_config').eq('id', fromPageId).single();
        if (page) {
          templateConfig = {
  ...
  const useTemplate = async (template: Template) => {
    try {
      ...
      const {
        data: copyResult,
        error: copyError
      } = await supabase.functions.invoke('copy-template-files', {
        body: {
          templateId: template.id,
  ...
  const renderTemplate = (template: Template) => <Card key={template.id} className="group hover:shadow-lg transition-shadow">
      <CardHeader>
        <div className="flex items-start justify-between">
          ...
          <div className="text-right text-sm text-muted-foreground">
            ...
            {template.conversion_rate && <div className="flex items-center gap-1">
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2">
            ...
            {template.industry_category && <Badge variant="secondary">{template.industry_category}</Badge>}
          ...
          <div className="flex gap-2">
            ...
            {template.preview_url && <Button variant="outline" size="sm" asChild>
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {showCreateButton && <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="template-name" value={newTemplate.name} onChange={e => setNewTemplate(prev => ({
                ...prev,
                name: e.target.value
                ...
                <div>
                  ...
                  <Textarea id="template-description" value={newTemplate.description} onChange={e => setNewTemplate(prev => ({
                ...prev,
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select onValueChange={value => setNewTemplate(prev => ({
                  ...prev,
                  category: value
                }))}>
                      <SelectTrigger>
      ...
      {loading ? <div className="flex items-center justify-center h-64">
        ...
        </div> : filteredTemplates.length === 0 ? <Card className="p-8 text-center">
          <CardContent>
            <div className="space-y-4">
              ...
              {showCreateButton && <Button onClick={() => setShowCreateDialog(true)}>
                  <Plus className="h-4 w-4 mr-2" />
